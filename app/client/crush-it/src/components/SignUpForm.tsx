  ...
  const [confirmPassword, setConfirmPassword] = useState('');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!passwordPattern.test(password)) {
            toast.error("Must have at least one lowercase and uppercase letter, one digit, one special char, and minimum 12 chars", { position: toast.POSITION.TOP_RIGHT });
        ...
        if (password === confirmPassword) {
            try {
                await axios.post('http://localhost:3000/api/register/register-user', { email, password });
                toast.success("Verification Email Sent!", { position: toast.POSITION.TOP_RIGHT });
            } catch (error: any) {
                if (error.response && error.response.status === 400) {
                    toast.error("User with this email already exists!", { position: toast.POSITION.TOP_RIGHT });
                } else {
