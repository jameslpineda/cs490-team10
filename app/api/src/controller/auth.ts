...
export const forgotPassword = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const resetUrl = coreConfig.frontendBaseUrl + '/reset-password/' + user?._id + '/' + resetToken;
...
export const resetPassword = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const user = await getUser({ _id: id, resetPasswordToken: token, resetPasswordExpireTime: { $gt: Date.now() } });
        if (!user) {
