...
export const resetPasswordValidation = (data: { id: string, token: string, password: string }) => {
    const schema = Joi.object({
        ...
        password: Joi.string()
            ...
            .custom((value, helpers) => {
                ...
                if (typesCount < 2) {
                    return helpers.message({ custom: 'Password must contain characters from at least two of the specified types: uppercase, lowercase, numeric, and special characters' });
                }
                return value;
            })
            .custom((value, helpers) => {
